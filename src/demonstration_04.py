...
def emotify(txt):
    ...
    # for k, v in data.items():
        ...
        # to return each new string with the new substrings substituted for the old one
        # txt = txt.replace(k,v)

    # return our new txt
    # return txt

    # Second pass solution Time Complexity O(1)
    # store the key value pairs of the required data for swapping out the text to emoticons
    # we will store this data in a hash table ( dictionary | object )
    data = {
        "smile" : ":)",
    ...
    # get the substring of txt from the start to index 7 (8 - 1) indices in [:8] set is as start_of_string -> "Make me "
    start_of_string = txt[:8]

    # get the data at 8 in from the start of the string [8: ] set it as end_of_string  -> ":)"
    end_of_string = txt[8:]
